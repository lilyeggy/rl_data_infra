...
class SftPackageIntegrityTest(unittest.TestCase):
    def test_verified_package_loads(self) -> None:
    ...
    def test_modified_training_data_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            _package(tmp_path)
    ...
    def test_modified_lora_config_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            _package(tmp_path)
            (tmp_path / "lora-config.json").write_text(json.dumps({"dataset": "other.jsonl"}))
            with self.assertRaisesRegex(ValueError, "LoRA config checksum"):
                _load_verified_package(tmp_path)

    def test_v6_package_requires_trainable_output_head(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            ...
            (tmp_path / "lora-config.json").write_text(json.dumps(config))
            (tmp_path / "training-package-manifest.json").write_text(json.dumps(manifest))
            with self.assertRaisesRegex(ValueError, "requires lm_head"):
                _load_verified_package(tmp_path)
