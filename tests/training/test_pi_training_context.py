...
class PiTrainingContextTest(unittest.TestCase):
    def test_capture_preserves_task_system_prompt_and_common_tools(self) -> None:
    ...
    def test_capture_rejects_tool_contract_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            one = tmp_path / "one.jsonl"
            two = tmp_path / "two.jsonl"
            ...
            with self.assertRaisesRegex(ValueError, "tool contract differs"):
                capture([f"task-one={one}", f"task-two={two}"])

    def test_qwen_runtime_messages_normalize_action_targets(self) -> None:
    ...
    def test_qwen_runtime_messages_reject_invalid_tool_arguments(self) -> None:
    ...
    def test_paired_tool_error_action_is_retained_as_target(self) -> None:
        row = {
    ...
    def test_gradient_accumulation_preserves_exact_epochs(self) -> None:
