...
def test_stdio_text_preserves_top_level_lines() -> None:
    assert _stdio_text(["1", "3", "4 7 2 9", "5 6 4 7"]) == (
        "1\n3\n4 7 2 9\n5 6 4 7"
    )


def test_stdio_text_joins_nested_values_with_spaces() -> None:
    assert _stdio_text([[1, 2], [3, 4]]) == "1 2\n3 4"


def test_verifier_handles_multiline_stdin_and_stdout(tmp_path: Path) -> None:
...
def test_verifier_rejects_call_based_task(tmp_path: Path) -> None:
