...
log="$repo/sft-runs/mbpp-post-sft-holdout-v2-260828.log"
exec >>"$log" 2>&1
...
echo "full_holdout_v2_start=$(date -Is)"
"$repo/scripts/serve_a6000_model.sh" base
run_eval "$mbpp" "$repo/holdout-v2-mbpp-base-260828" "$repo/mbpp-workspaces-v2-base" "$base_revision" 90
run_eval "$humaneval" "$repo/holdout-v2-humaneval-base-260828" "$repo/humaneval-workspaces-v2-base" "$base_revision" 164
...
run_eval "$mbpp" "$repo/holdout-v2-mbpp-candidate-260828" "$repo/mbpp-workspaces-v2-candidate" "$candidate_revision" 90
run_eval "$humaneval" "$repo/holdout-v2-humaneval-candidate-260828" "$repo/humaneval-workspaces-v2-candidate" "$candidate_revision" 164
echo "full_holdout_v2_finished=$(date -Is)"
