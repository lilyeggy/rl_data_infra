...
Key optimizations:
- Removed inner-loop torch.cuda.empty_cache() (eliminates CPU-GPU barrier stalls)
- Enabled TF32 on Ampere / Blackwell architectures
- Increased chunked CE loss chunk size to 4096
- Length-bucketed dynamic batching with SDPA attention
- Clean resume support from intermediate checkpoints (e.g. epoch0)
"""
...
# Enable TF32 for Tensor Core acceleration on Ampere / Blackwell
...
def chunked_completion_loss(
    ...
    """CE only on completion positions, computed in fp32 chunks."""
...
def main() -> None:
    ...
    step = args.start_epoch * len(batches)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for bi, b_idx in enumerate(order):
            ...
            recent_tokens += real_tokens
            step += 1

            if step % args.log_every == 0 or bi == 0:
                elapsed = time.monotonic() - run_start
                recent_elapsed = time.monotonic() - recent_start
                recent_tps = recent_tokens / max(0.001, recent_elapsed)
                remaining_steps = total_steps - step
                steps_done_in_run = max(1, step - args.start_epoch * len(batches))
                sec_per_step = elapsed / steps_done_in_run
                ...
                print(
                    ...
                    f"speed={recent_tps:.0f} tok/s eta={eta / 60:.0f}min",
                    flush=True,
                )
                recent_tokens = 0
                recent_start = time.monotonic()
        ...
        # Flush cache only at epoch boundary
