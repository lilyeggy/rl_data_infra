# Multi-Harness Agent Execution Data Plane（旧版 README，已归档）
...
- V2.1 bounded `FILE_NOT_FOUND` recovery policy、Hook decision lineage、runtime controller 和 derived replay report；
- V2.2 explicit Multi-Agent causal graph 与 per-agent training eligibility view；
- V2.3 partitioned append-only ingestion/storage baseline；
- 3 对真实 error-recovery reference case：control 0/3、candidate 3/3，但因 token/latency 超预算，Gate 诚实输出 `REJECT`；V2.1 replay 不伪造 runtime `HARNESS_DECISION`。
...
# Real Pi experiment (requires pi 0.84.2 + provider credentials + a workspace)
python3 -m src.cli run-real-experiment \
  --output artifacts/v2.1-live --model deepseek-v4-flash
# Optional Linux/container smoke test; requires Docker daemon
...
- V2.1 深层 Trace 与决策可观测性：[docs/learning/v2.1-decision-observability-guide.md](docs/learning/v2.1-decision-observability-guide.md)
- V2.1 进展与 replay：[docs/releases/v2.1-decision-observability.md](docs/releases/v2.1-decision-observability.md)
- V2.1 真实服务器实验：[docs/releases/v2.1-live-server.md](docs/releases/v2.1-live-server.md)
- V2.4 离线训练视图（SFT/偏好对）：[docs/releases/v2.4-offline-training-view.md](docs/releases/v2.4-offline-training-view.md)
- 本地模型实验记录（基线/SFT/泛化/GRPO）：[docs/releases/v2.5-local-model-experiments.md](docs/releases/v2.5-local-model-experiments.md)
- GPU 服务器部署 runbook：[docs/runbook-gpu-server.md](docs/runbook-gpu-server.md)
- 本地模型实验脚本：[experiments/local_model/](experiments/local_model/README.md)
- V2.2 Multi-Agent 语义：[docs/learning/v2.2-multi-agent-semantics-guide.md](docs/learning/v2.2-multi-agent-semantics-guide.md)
- V2.2 release：[docs/releases/v2.2-multi-agent-semantics.md](docs/releases/v2.2-multi-agent-semantics.md)
- V2.3 生产数据面：[docs/learning/v2.3-production-data-plane-guide.md](docs/learning/v2.3-production-data-plane-guide.md)
- V2.3 release：[docs/releases/v2.3-production-data-plane.md](docs/releases/v2.3-production-data-plane.md)
