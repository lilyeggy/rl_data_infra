# Day 6：Harness Evaluation、Failure Attribution 与 Regression Gate（已归档）
...
```text
状态：COMPLETED（V2 真实 Pi reference case；当前 Gate 为 REJECT）
metric版本：episode-metrics/v1
归因规则版本：failure-attribution/v1
paired episodes：3 对，coverage 100%
reference case：FILE_NOT_FOUND recovery，OBSERVED_TOOL_ERROR_LOOP 3→0
Gate结果：REJECT；token +43.54%，latency +51.15% 超过冻结阈值
限制：黑盒 Pi 未暴露完整 Harness decision；3 对样本只证明 reference mechanism
```
