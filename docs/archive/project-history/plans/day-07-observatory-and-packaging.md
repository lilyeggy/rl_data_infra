# Day 7：Harness Observatory、Reference Improvement 与项目包装（已归档）
...
```text
状态：COMPLETED（V2 Observatory、TrainingCandidateView 与 release package 已交付）
UI入口：observatory.html（由 v2 artifact package 生成）
control/candidate：真实 Pi 0.84.2 / gpt-5.6-luna，3 对
paired episodes：3 对，coverage 100%
Gate verdict：REJECT；成功率改善但 token/latency 超预算
测试：V2 baseline 全量测试通过（Python 3.10）
演示时长：以本地只读 artifact package 为准
最终限制：小样本、黑盒 Harness decision、训练视图不是 on-policy RL rollout
```
