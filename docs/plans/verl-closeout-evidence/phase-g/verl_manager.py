...
class CertifiedVerlAgentLoopManager(AgentLoopManager):
    ...
    async def generate_sequences(self, prompts: Any) -> Any:
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                last_error = exc
                logger.warning(
                    "%s: attempt %d produced no certifiable batch (%s); resampling",
                    self.run_id,
                    attempt,
                    exc,
                )
            ...
            self._record_certificate(certified, attempt)
            logger.info(
    ...
    def _record_certificate(self, certified: CertifiedBatch, attempt: int) -> None:
        ...
        (self.episode_root / f"certified-batch-attempt{attempt}.json").write_text(
