...
class PiAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs: Any) -> AgentLoopOutput:
        ...
        model_id = _served_model_id(engine_base)
    ...
    def _execute_episode(
        ...
        *,
        ...
        policy_artifact_checksum = rl_decision.get("policy_artifact_checksum") or "0" * 64
...
def _agent_prompt(task_id: str) -> str:
    # Two format requirements, both forced by the certified data plane:
    #
    # * The tool-call wire format has to be stated. Qwen2.5's chat template
    #   does print the <tool_call> instruction into the system message, but the
    #   policy here emits the bare JSON object instead, and the server-side
    #   parsers (vLLM's hermes, and the template's own contract) only recognise
    #   the tagged form. The stage E server accepted untagged JSON because it
    #   did its own permissive extraction; stock vLLM does not, so the prompt
    #   has to make the format explicit.
    # * The whole task has to be answered in one response. The assembler
    #   requires each call's prompt to be the previous prompt followed by the
    #   previous generation, which is what makes the assembled sequence exactly
    #   the context the model saw. Pi re-renders the assistant turn from its
    #   structured tool calls instead of echoing the generation, so episodes
    #   that span several calls cannot be certified. Asking for the read /
    #   write / test sequence in a single response keeps episodes inside the
    #   contract, and is the shape the stage E episodes were certified in.
    return (
        "You are in a workspace with solution.py. In ONE response, emit the "
        "complete sequence of tool calls needed to finish this task: read "
        "task-prompt.txt, edit solution.py to implement the requested "
        "function, then run it with bash. Emit all of them together and do "
        "not wait for results in between. Keep reasoning brief. Emit every "
